...
class LLMFetchOps:
    ...
    def _fetch_raw_reactions(self, chem, mode=None):
        if mode is None:
            mode = "documented_rp"
        ...
        for curr_model in models_schedule:
            response = self.fetch_llm_answer_str(instruct, curr_model)
        ...
        self.logger.log(
            f"Got {len(reactions_revalid)} reactions for '{chem_name}' with '{model}'; "
            f"CTT: {self.llm_client.completion_tokens_total}"
        )
        return {'cid': chem['cid'], 'reactions': reactions_revalid}

    def _get_raw_reactions(self, out_fn, max_workers, mode=None, criteria=None):
        criteria = (lambda _: True) if criteria is None else criteria
        processed = self._get_processed_entries(out_fn, 'cid')
        chems_staged = [
            chem for chem in self.compounds.chems if chem['cid'] not in processed and criteria(chem)
        ]
        self._submit_entries_to_llm(
            out_fn,
            chems_staged,
            max_workers,
            self._fetch_raw_reactions,
            routine_args=[mode],
            description="Reactions generation",
        )

    def get_raw_reactions(self, max_workers=1):
        self._get_raw_reactions(self.reaction_parser.raw_reactions_fn, max_workers)

    def get_uncommon_raw_reactions_for_wiki_chems(self, max_workers=1):
        self._get_raw_reactions(
            self.reaction_parser.wiki_raw_reactions_fn,
            max_workers,
            mode="documented_less_common_rp",
            criteria=lambda chem: chem['wiki'],
        )

    def get_rare_raw_reactions_for_top_chems(self, max_workers=1):
        hazards_chems = {}
        with open(self.compounds.chems_hazards_wiki_fn) as f:
            for line in f:
        ...
        def criteria(chem):
            ...
            for pic in hazards_chems[cid]['pictograms']:
                if pic in {'GHS01', 'GHS03', 'GHS06'}:
                    return True
            return False

        self._get_raw_reactions(
            self.reaction_parser.top_rare_raw_reactions_fn,
            max_workers,
            mode="rare_rp",
            criteria=criteria,
        )
    ...
    def get_uncommon_raw_reactions_for_wiki_chems_products_only(self, max_workers=1):
        self._get_raw_reactions(
            self.reaction_parser.products_wiki_raw_reactions_fn,
            max_workers,
            mode="documented_less_common_p",
            criteria=lambda chem: chem['wiki'],
        )

    def get_uncommon_raw_reactions_for_annotated_chems_products_only(self, max_workers=1):
        self._get_raw_reactions(
            self.reaction_parser.products_annot_raw_reactions_fn,
            max_workers,
            mode="documented_less_common_p",
            criteria=lambda chem: 'annotation' in chem and not chem['wiki'],
        )
    ...
    def _validate_raw_reactions_batch(self, reactions, valid_cnt):
        instruct_validate = (
        ...
        for try_i, model in enumerate(models_schedule):
            ...
            while valid_i < valid_cnt and len(staged) > 0:
                reactions_str = '\n'.join(
                    [f"{i+1}. {react['reaction']}" for i, react in enumerate(staged)]
                )
                ...
                if len(verdicts) != len(staged):
                    if mistakes_cnt == mistakes_thr:
                        self.logger.log_warn(
                            f"Falling to another model due to mistakes ({try_i+1}/{len(models_schedule)}) "
                            f"('{model}')"
                        )
                        bad = True
                        break
                ...
                for i in range(len(verdicts)):
                    ...
                    if confidences[i] >= confidence_thr or max_confidence <= confidence_thr:
                        ...
                        self.logger.log(
                            f"Processed reaction '{reaction_str}'; confidence: {est_confidence:.2f}; "
                            f"CTT: {self.llm_client.completion_tokens_total}"
                        )
    ...
    def validate_raw_reactions(self, raw_reactions_fn=None, max_workers=1):
        if raw_reactions_fn is None:
            raw_reactions_fn = self.reaction_parser.raw_reactions_fn

        def get_reaction_id(react):
            react_parsed, _ = self.reaction_parser.parse_reaction_scheme(react['reaction'])
            if not react_parsed:
                self.logger.log_warn(f"Verdict file contains non-parsable reaction: '{react}'")
                return None
            return react_parsed['rid']

        processed_rids = self._get_processed_entries(self.reaction_parser.raw_reactions_verdict_fn, key=get_reaction_id)

        entries = self.store.load_jsonl(raw_reactions_fn)
        reactions = []
        for entry in entries:
            reactions_curr = entry['reactions'] or []
            for react in reactions_curr:
                react_parsed, _ = self.reaction_parser.parse_reaction_scheme(react)
        ...
        self._submit_entries_to_llm(
            self.reaction_parser.raw_reactions_verdict_fn,
            reactions,
            max_workers,
            self._validate_raw_reactions_batch,
            routine_args=[9],
            batch_size=10,
            description="Reactions validation",
        )
